...
const SelectCompanyBottomSheet = forwardRef((props, ref) => {
    ...
    const fetchCompanies = (pageNum, searchQuery = searchText) => {
        if (pageNum === 1) setIsLoading(true);
        makeRequest({
            url: `/companies/selectlist?search=${searchQuery}&page=${pageNum}`,
            method: 'GET',
    ...
    const selectCompany = (item) => {
        props.setSelectedCompany({ ...item, name: item.text });
        ref.current.close();
    };
    ...
    return (
        <BottomSheetModal
            ...
            onDismiss={() => { setSearchText(''); setCompanies([]); }}
        >
            <BottomSheetFlatList
                data={companies}
