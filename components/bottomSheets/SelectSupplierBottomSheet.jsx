...
const SelectSupplierBottomSheet = forwardRef((props, ref) => {
    ...
    const fetchSuppliers = (pageNum, searchQuery = searchText) => {
        if (pageNum === 1) setIsLoading(true);
        makeRequest({
            url: `/suppliers/selectlist?search=${searchQuery}&page=${pageNum}`,
            method: 'GET',
            ...
            .then((res) => {
                ...
                setSuppliers(prev => pageNum === 1 ? newItems : [...prev, ...newItems]);
    ...
    const loadMore = () => {
        ...
        pageRef.current += 1;
        fetchSuppliers(pageRef.current);
    };

    const handleSheetAnimate = (fromIndex, toIndex) => {
        if (fromIndex < 0 && toIndex >= 0) {
            pageRef.current = 1;
            fetchSuppliers(1, searchText);
        }
    };
    ...
    return (
        <BottomSheetModal
            ...
            onDismiss={() => { setSearchText(''); setSuppliers([]); }}
        >
            <BottomSheetFlatList
                data={suppliers}
